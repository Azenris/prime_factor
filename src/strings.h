...
inline char *string_utf8_clone( MemoryArena *memory, const char *source )
{
	u64 bytes = string_utf8_bytes( source );
	char *clone = (char *)memory_arena_transient_allocate( memory, bytes );
...
[[nodiscard]] inline const char *string_utf8_get_path( const char *str, MemoryArena *memory )
{
	u64 bytes = string_utf8_bytes( str );
	char *dest = (char *)memory_arena_transient_allocate( memory, bytes );
